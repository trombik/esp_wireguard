...
static err_t initiate_new_connection(struct wireguard_device *dev);
static err_t shutdown_connection(struct wireguard_device *dev);
...
void derp_tick(struct wireguard_device *dev) {
	...
	if (is_any_peer_active && dev->derp.tls == NULL && connection_attempt_control == AttemptNow) {
		...
		err = initiate_new_connection(dev);
...
static void read_from_interface_worker(void *arg) {
	...
	struct derp_pkt * packet_ptr;

	ESP_LOGI(TAG, "Read from interface worker starting");

	while (1) {
		xTaskNotifyWaitIndexed(0, ULONG_MAX, ULONG_MAX, (uint32_t*)&packet_ptr, portMAX_DELAY);
		struct derp_pkt *data_pkt = packet_ptr;

		ESP_LOGD(TAG, "Attempting to send data packet ...");
...
static void read_from_network_worker(void *arg) {
	...
	while (1) {
		ssize_t read_len = esp_tls_conn_read(dev->derp.tls, data_pkt, max_data_pkt_size);
		if (read_len <= 0) {
			ESP_LOGE(TAG, "Failed to read from network: %02x", read_len);
			goto end;
		}

		if (read_len < 5 || read_len < 5 + BE32_TO_LE32(data_pkt->length_be)) {
			ESP_LOGE(TAG, "Received packet too short, len=%d", read_len);
			continue;
		}

		if (data_pkt->type != 0x05) {
			ESP_LOGW(TAG, "Invalid packet type=%02x", data_pkt->type);
			continue;
		}

		if (data_pkt->data.subtype != 0x00) {
			ESP_LOGW(TAG, "Invalid packet subtype=%02x", data_pkt->data.subtype);
			continue;
		}
		...
		// Find which peer packet is being sent, we will assign port according to this
		int index = -1;
		for (int i = 0; i < WIREGUARD_MAX_PEERS; i++) {
		...
		if (index == -1) {
			ESP_LOGW(TAG, "Received data mesage for unknown peer. Ignoring ...");
			continue;
		}

		uint16_t port = 49152 + index;
		ESP_LOGD(TAG, "Invoking packet receiving callback for wireguard for peer on port %d", port);
...
void derp_set_endpoint(struct wireguard_device *dev, const char* host, uint16_t port) {
	ESP_LOGI(TAG, "Updating derp with %s", host);
	dev->derp.endpoint.addr_len = strlen(host);
	memcpy(dev->derp.endpoint.addr, host, dev->derp.endpoint.addr_len);
	dev->derp.endpoint.addr[dev->derp.endpoint.addr_len] = '\0';
	dev->derp.endpoint.port = port;
	connection_attempt_control = AttemptNow;
}

err_t initiate_new_connection(struct wireguard_device *dev) {
	LWIP_ASSERT("derp_tick: invalid state", dev->derp.conn_state == CONN_STATE_TCP_DISCONNECTED);
	LWIP_ASSERT("initiate_new_connection: invalid state", dev->derp.tls == NULL);
...
err_t shutdown_connection(struct wireguard_device *dev) {
	if (dev->derp.tls != NULL) {
